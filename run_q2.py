    ...
    if len(v.shape) == 2:
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                v[i, j] += eps
                loss_forward = compute_loss(x, y,  params)
                v[i, j] -= eps * 2
                loss_backward = compute_loss(x, y, params)
                v[i, j] += eps
                grad[i, j] = (loss_forward-loss_backward)/(eps*2)
    else:
        for i in range(v.shape[0]):
            v[i] += eps
            loss_forward = compute_loss(x, y,  params)
            ...
            loss_backward = compute_loss(x, y, params)
            v[i] += eps
            grad[i] = (loss_forward-loss_backward)/(eps*2)
